...
import { BrainCircuit, Clock3, ExternalLink, Link2, Loader2, ShieldCheck, ShieldAlert, Sparkles, XCircle } from "lucide-react";
...
type CaseDetailPayload = {
  id: number; invoiceId: number; status: string; stage: string; riskScore: number; paymentProbability: number; amountAtRisk: number; recoveredAmount: number; expectedRecovery: number; lastDecision: string | null; nextActionAt: string | null;
...
export function RecoveryCaseDetail({ open, onClose, detail, onApprove, onCreatePaymentLink }: { open: boolean; onClose: () => void; detail: CaseDetailPayload | null; loading?: boolean; onApprove: (caseId: number) => Promise<void>; onCreatePaymentLink: (invoiceId: number) => Promise<string | null> }) {
  ...
  const createPaymentLink = async () => {
    ...
    try {
      setPaymentLinkUrl(await onCreatePaymentLink(detail.invoiceId));
    } finally {
  ...
  return <Dialog open={open} onOpenChange={(visible) => !visible && onClose()}>
    <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
      ...
      <div className="space-y-5 text-sm">
        ...
        {invoice.balance > 0 && detail.status !== "PAID" && <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-emerald-950">Razorpay Test Mode checkout</p><p className="text-xs text-emerald-800 mt-1">Create one live Test Mode payment link for this outstanding balance. Razorpay will send it to the seeded demo recipient only.</p></div>{paymentLinkUrl ? <a href={paymentLinkUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-800"><ExternalLink className="h-4 w-4" />Open Test Checkout</a> : <Button size="sm" onClick={createPaymentLink} disabled={creatingPaymentLink} className="gap-2 shrink-0 bg-emerald-700 hover:bg-emerald-800">{creatingPaymentLink ? <Loader2 className="h-4 w-4 animate-spin" /> : <Link2 className="h-4 w-4" />}Create Test Mode Link</Button>}</div>{paymentLinkUrl && <p className="mt-3 text-xs text-emerald-800">Link created and stored against this invoice. Completing payment in Razorpay Test Mode will trigger the signed webhook and close this case.</p>}</div>}
